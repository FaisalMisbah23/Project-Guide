# Chapter 03 - Design the data model

> A portfolio becomes a real app when its content has a durable shape. Projects, articles, comments, contact messages, and analytics need tables, constraints, seed data, and migrations that can be explained later.
...
## 03.02 - Why This Feature Matters

Portfolio database matters because it changes how the portfolio behaves for real users. It is not a decorative layer; it affects what visitors can see, what the owner can manage, and what proof the final demo can show.
...
## 03.05 - The Professional Approach

The professional path is to model content that needs filtering, protection, and ownership as clear tables with constraints. This is the required path for the course because every later chapter assumes the same shape.
...
## 03.06 - Key Concepts In Plain English
...
## 03.12 - Write The First Screen Or Helper

Build the first visible or reusable piece for portfolio database. Keep it intentionally small: one screen shell, one helper contract, or one component boundary.
...
## 03.15 - Add Error And Validation State

Add error and validation states for portfolio database. A user should never have to guess whether the app is waiting, empty, broken, or rejecting their input.
...
For `portfolio database`, the query or function should respect:
...
## 03.17 - Protect Private Data

Protect the private side of portfolio database. Browser checks improve experience, but the database or server boundary must enforce the rule.
...
## 03.18 - Add The Admin Or Public Ui
...
For this chapter, the UI should make `Supabase schema` obvious without explaining the course itself on screen.
...
## 03.22 - Write The Learning Log

Create or update `learning-log/03-data-model-and-migrations.md`.

Answer these in your own words:

```txt
What did Chapter 03 add to the project?
...
## 03.23 - Recap And Whats Next

You added Supabase schema and proved it with visible checks. The important lesson is not only that the feature works; it is that you can explain why this shape is safer than the shortcut.

Next, Chapter 04 builds RLS policies.
