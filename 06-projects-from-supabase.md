# Chapter 06 - Load projects from Supabase
...
## 06.02 - Why This Feature Matters

Published projects matters because it changes how the portfolio behaves for real users. It is not a decorative layer; it affects what visitors can see, what the owner can manage, and what proof the final demo can show.
...
## 06.04 - The Beginner Approach And Its Cost

The tempting beginner move is to fetch all projects and filter drafts inside React. It feels fast because it removes planning, but the cost appears later when the app must handle real data, privacy, or production checks.
...
## 06.05 - The Professional Approach

The professional path is to query only published projects and let RLS enforce the same rule. This is the required path for the course because every later chapter assumes the same shape.
...
## 06.06 - Key Concepts In Plain English
...
## 06.12 - Write The First Screen Or Helper

Build the first visible or reusable piece for published projects. Keep it intentionally small: one screen shell, one helper contract, or one component boundary.
...
## 06.13 - Wire The Route Or Module
...
## 06.14 - Add Loading And Empty State

Add loading and empty states for published projects. A user should never have to guess whether the app is waiting, empty, broken, or rejecting their input.
...
## 06.15 - Add Error And Validation State

Add error and validation states for published projects. A user should never have to guess whether the app is waiting, empty, broken, or rejecting their input.
...
For `published projects`, the query or function should respect:
...
## 06.18 - Add The Admin Or Public Ui
...
For this chapter, the UI should make `project list and detail pages` obvious without explaining the course itself on screen.
...
## 06.22 - Write The Learning Log

Create or update `learning-log/06-projects-from-supabase.md`.

Answer these in your own words:

```txt
What did Chapter 06 add to the project?
